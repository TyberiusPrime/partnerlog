...
This key is for debug APKs only and must not be reused for production signing. The build requires `partnerlogDebugKeystorePassword` and `partnerlogDebugKeyPassword` Gradle properties (workflows currently set both to `android`).
